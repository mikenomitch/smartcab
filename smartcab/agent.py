...
class LearningAgent(Agent):
    ...
    def get_maxQ(self, state):
        ...
        for actionKey in actionsForState:
            if maxQ and maxQ < actionsForState[actionKey]:
                maxQ = actionsForState[actionKey]
    ...
    def learn(self, state, action, reward):
        ...
        # told to not use 'gamma', so just set it to one
        discount_factor = 1

        old_q_value = self.Q[state][action]
        new_q_value_sans_weighting = reward + (discount_factor * self.get_maxQ(state))
        new_q_value = old_q_value + (learning_rate * new_q_value_sans_weighting)
